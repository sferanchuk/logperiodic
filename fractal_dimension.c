...
void higuchi_dimension( double *data_x, double *data_y, int npoints, int method, int interval_min, int interval_max, int extra_output, double *slope, double *intercept, double *correlation )
{
    ...
    double dx, dy, partial_sum, t_slope, t_intercept;
    ...
    *correlation = linear_regression( log_intervals, log_path, num_intervals, &t_slope, &t_intercept );
...
void peng_dimension( double *data_x, double *data_y, int npoints, int interval_min, int interval_max, int extra_output, double *slope, double *intercept, double *correlation )
{
    int i, j, k, npasses;
    int num_intervals, interval;
...
int logperiodic_approximation( double *x_coord, double *y_coord, int num_points, int method, int interval_min, int interval_max, int extra_output, int *first_period, int *last_period, double *correlation )
{
    int i, j, k;
    int direction;
    double peak_at_left, peak_at_right, sum_of_intervals;
    ...
    for ( direction = 0; direction <= 1; direction++ )
    {
        ...
        {
            if ( extra_output ) printf( "[min_period=%d,%s]", i, ( direction ) ? "forwards" : "backwards" );
            for ( j = 2; j < 100; j = ( j * 3 ) / 2 )
            {
                ...
                if ( method == 0 ) peng_dimension( updated_x_coord, y_coord, num_points, interval_min, interval_max, 0, &t_slope, &t_intercept, &t_correlation );
                else higuchi_dimension( updated_x_coord, y_coord, num_points, 1, interval_min, interval_max, 0, &t_slope, &t_intercept, &t_correlation );
    ...
    if ( method == 0 ) peng_dimension( x_coord, y_coord, num_points, interval_min, interval_max, 0, &t_slope, &t_intercept, &t_correlation );
    else higuchi_dimension( x_coord, y_coord, num_points, 1, interval_min, interval_max, 0, &t_slope, &t_intercept, &t_correlation );
...
int main( int argc, char **argv )
{
    ...
    double *ptr;
    double correlation = 0, dimension, intercept;
    ...
    if ( npoints > 0 && argv[1][0] == '-' ) 
    {
        ...
        switch ( argv[1][1] )
        {
            case 'h':
            {
                higuchi_dimension( xvalues, yvalues, npoints, ( argv[1][2] == 'm' ) ? 1 : 0, interval_min, interval_max, extra_output, &dimension, &intercept, &correlation );
                printf( "dimension: %g\nintercept: %g\ncorrelation: %g\n", dimension, intercept, correlation );        
                break;
            }
            case 'p':
            {
                peng_dimension( xvalues, yvalues, npoints, interval_min, interval_max, extra_output, &dimension, &intercept, &correlation );
                printf( "dimension: %g\nintercept: %g\ncorrelation: %g\n", dimension, intercept, correlation );        
                break;
            }
            case 'c':
            case 'd':
            {
                credibility = logperiodic_approximation( xvalues, yvalues, npoints, argv[1][1] == 'c', interval_min, interval_max, extra_output, &first_period, &last_period, &correlation );
